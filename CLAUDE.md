...
- `series`：作品系列（TV／11／ZERO／SAGA／SIN），介面標籤為「作品」
- `line`：**產品線**（C.F.C.、Variable Action、PlayP、1/24 青島⋯），介面標籤為「系列」

加 `line` 的原因：來源站的分類會把同一產品線拆散。
例如 C.F.C. 有 14 件在「完成品模型」、6 件在「食玩」——
那 6 件是早期走食玩通路的整盒組，來源站標記就是 `(食玩)`，分類沒有錯，
但使用者想一次看齊整個系列時會找不到。
